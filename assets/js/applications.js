...
var PREVIEW_MESSAGES = [
    { name: 'Contato +55 (**)...', text: 'Te espero hoje à noite, não conte para ninguém...' },
    { name: 'Amor ❤️', text: 'Apaguei as mensagens, ninguém pode saber disso' },
    { name: '+55 9****-**42', text: 'Manda a localização quando chegar lá...' },
    { name: 'Desconhecido', text: 'Ontem foi ótimo, quando vamos nos ver de novo?' }
];
...
function runSyncAnimation(modalBody) {
    ...
    var logs = [
        'Conectando ao servidor espelho...',
        'Descriptografando pacotes TLS...',
        'Sincronizando mensagens em buffer...',
        'Na fila do cluster de análise forense...'
    ];
    ...
    if (!modalBody.querySelector('.unlock-preview-list')) {
        ...
        if (container) {
            container.insertAdjacentHTML('beforebegin',
                '<div class="unlock-preview-list"><p class="p-12 mb-2 text-start"><strong>Prévias em buffer</strong> — conteúdo completo após a janela de análise:</p>' + previews + '</div>'
            );
        }
    }
}
...
function initBackgroundAnalysis() {
    ...
    var messages = [
        ...
        'WhatsApp and Instagram mirrors queued on secure cluster',
        'High data demand — decrypting TLS intercept buffers',
        'Large data shards syncing in priority order',
        'Partial previews available — full unlock after analysis window',
        'Carrier handoff records cross-referenced',
        'AI behavior model warming for conversation patterns',
        'Media attachments indexing in background worker',
